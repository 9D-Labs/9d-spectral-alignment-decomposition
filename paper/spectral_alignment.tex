...
\begin{abstract}
We study Hessian \emph{directional curvature}---the Rayleigh quotient $h_k = \vecop(u_k v_k^\top)^\top \mathcal{H}\, \vecop(u_k v_k^\top)$---along the singular directions of layer gradients. Empirically, $h_k$ obeys a local spectral law $h_k \propto \sigma_k^\alpha$, with $\alpha$ varying systematically across layer types ($\alpha \approx 2$ for convolutions, $\approx 1$ for transformer attention, $< 1$ for MLP up-projections). Under the Gauss--Newton (GN) Kronecker curvature model, we derive the \textbf{Spectral Alignment Decomposition}: $\alpha = d\log\Lambda_k/d\log\sigma_k + d\log\Phi_k/d\log\sigma_k$, where the first term captures covariance-spectral scaling and the second measures alignment between Kronecker factor eigenbases and gradient singular directions. When eigenbases align, the first term is~2 and deviations from $\alpha = 2$ reduce to a geometric question we answer for LayerNorm, residual connections, and softmax heads. The decomposition implies a finite-window \textbf{spectral transfer relation} $s \approx \alpha\gamma$ linking the curvature exponent, an effective gradient rank-decay $\gamma$, and directional curvature decay $s$. The relation is algebraic; its empirical content is that $\alpha$ and $\gamma$, fit on \emph{independent} data (Hessian-vector products vs.\ SVD), recover $s$ to $\approx$2\% median error across 93 layers, five architectures, and three datasets---with no free parameters. On the measured top-$k$ window, the curvature participation ratio is low ($\mathrm{PR}_h \in [1.0, 1.8]$), consistent with a zeta-function bound. As a proof of concept, we derive the architecture-adaptive preconditioner $T(\sigma;\alpha)$ and show that \textbf{Spectral Newton}---implementing $T$ in the gradient singular basis---outperforms AdamW on vision benchmarks where $\alpha \approx 2$.
\end{abstract}
...
We study curvature \emph{along gradient singular directions}. For layer $\ell$ with gradient $G_\ell = U\Sigma V^\top$ (singular value decomposition, SVD), let $h_k$ denote the directional curvature (Rayleigh quotient) $h_k = \vecop(u_k v_k^\top)^\top \mathcal{H} \vecop(u_k v_k^\top)$ along the rank-one direction $u_k v_k^\top$, measured via Hessian-vector products (HVPs). Note that $h_k$ is not a Hessian eigenvalue---$u_k v_k^\top$ is a gradient singular direction, not generally a Hessian eigenvector---but it measures the curvature the optimizer \emph{actually encounters}. Empirically, $h_k$ follows a power law in the gradient singular value $\sigma_k$:
...
Our central contribution is the \textbf{Spectral Alignment Decomposition} (Theorem~\ref{thm:spectral-alignment}): an identity decomposing $h_k$ into Kronecker factor eigenvalues and measurable alignment ratios under the GN Kronecker curvature model. The decomposition is an accounting identity by construction; its explanatory power comes from architectural predictions about the slopes of its terms (Sections~\ref{sec:mechanisms}--\ref{sec:spectral}). We then:
\begin{enumerate}[leftmargin=*,nosep]
    ...
    \item Establish the \textbf{spectral transfer relation} $s \approx \alpha\gamma$: an algebraic link between $\alpha$, an effective rank-decay exponent $\gamma$ fit on gradient singular values, and directional curvature decay $s$~\cite{tang2025hessian}, validated on 89 layers across three datasets (Section~\ref{sec:spectral}).
    \item Bound the curvature participation ratio (PR) via the Riemann zeta function; empirically, $\mathrm{PR}_h \in [1.0, 1.8]$ on the measured top-$k$ window, indicating curvature concentrates onto a few gradient singular directions.
...
\begin{enumerate}[leftmargin=*,nosep]
    \item \textbf{(Decomposition identity.)}
    $h_k = \rho_k^{(\delta)} \cdot \rho_k^{(A)} \cdot [\Lambda_\delta]_{kk} \cdot [\Lambda_A]_{kk}$.

    This is an accounting identity given the Kronecker curvature model; its explanatory power comes from the empirical predictions below.
    \item \textbf{($\alpha$ as alignment slope.)} With $\Phi_k = \rho_k^{(\delta)}\rho_k^{(A)}$, $\beta = d\log\Lambda_k / d\log\sigma_k$ (the covariance-spectral exponent), and power-law fit $h_k = c\sigma_k^\alpha$:
    \begin{equation}\label{eq:alpha-decomp}
        \alpha = \underbrace{\frac{d \log \Lambda_k}{d \log \sigma_k}}_{\displaystyle\beta} + \underbrace{\frac{d \log \Phi_k}{d \log \sigma_k}}_{\displaystyle\phi}, \qquad \Lambda_k = [\Lambda_\delta]_{kk}[\Lambda_A]_{kk}
    \end{equation}
    The special case $\beta = 2$ holds when Kronecker factor eigenbases align with gradient singular directions (Theorem~\ref{thm:curvature-baseline}); this is empirically typical for convolutional layers.
    \item \textbf{(Canonical form.)} When $Q_\delta = U$, $Q_A = V$ (perfect alignment), $[\Lambda_\delta]_{kk}[\Lambda_A]_{kk} = \sigma_k^2/\chi_k$ and
    \begin{equation}\label{eq:alpha-general}
        \alpha = 2 + \frac{d \log \Phi_k}{d \log \sigma_k} - \frac{d \log \chi_k}{d \log \sigma_k}
    \end{equation}
    where $\chi_k = h_k^{\mathrm{exact}}/h_k^{\mathrm{Kron}}$ is the \emph{curvature ratio} measuring the GN-to-exact gap. Note $\chi_k$ can exceed~1 (Section~\ref{sec:conv-gap}).
\end{enumerate}
\end{theorem}
...
\textbf{Part 3.} When $Q_\delta = U$ and $Q_A = V$, we have $\rho_k^{(\delta)} = \rho_k^{(A)} = 1$, so $\Phi_k = 1$. Under the alignment condition of Theorem~\ref{thm:curvature-baseline}, $[\Lambda_\delta]_{kk} [\Lambda_A]_{kk} = \sigma_k^2 / \chi_k$, hence $d\log\Lambda_k / d\log\sigma_k = 2 - d\log\chi_k / d\log\sigma_k$. Substituting into Part~2 with $d\log\Phi_k = 0$ gives Eq.~\eqref{eq:alpha-general}.
\end{proof}

\begin{remark}
Equation~\eqref{eq:alpha-general} is the central result: $\alpha$ deviates from $2$ iff $\Phi_k$ or $\chi_k$ correlate with $\sigma_k$. When both are $k$-independent, $\alpha = 2$ regardless of their magnitudes. The decomposition is an identity under the GN/Kronecker curvature model---its predictive value lies in the fact that $\beta$, $\phi$, and $\chi_k$ are independently measurable and architecturally interpretable (Sections~\ref{sec:mechanisms}--\ref{sec:spectral}).
\end{remark}
...
\begin{proposition}[LayerNorm flattens activation alignment (idealized model)]\label{thm:layernorm}
Under an isotropic random-feature model for normalized activations (sub-Gaussian, weakly correlated features): for $y = W \cdot \mathrm{LN}(x)$, the activation covariance $C_A^{\mathrm{LN}} = \Gamma \hat{C} \Gamma$ with $\hat{C}$ the normalized-input covariance. The condition number $\kappa(\hat{C}|_{\mathbf{1}^\perp}) = 1 + O(\sqrt{n/B})$ by Bai--Yin~\cite{baiyin1993}, and $\rho_k^{(A)}$ is confined to a $\sigma_k$-independent band. Thus $d\log\rho_k^{(A)}/d\log\sigma_k \approx 0$, reducing $\alpha$ from $2$ toward $1$. Real transformer activations have richer correlation structure; the proposition provides an idealized baseline that we validate empirically in Appendix~\ref{app:ablations}.
\end{proposition}

\begin{corollary}\label{cor:ln-alpha}
With $\chi_k$ approximately constant, $\alpha \approx 1 + d\log\rho_k^{(\delta)}/d\log\sigma_k$. If $u_k^\top C_\delta u_k \propto \sigma_k$, then $\alpha \approx 1$.
\end{corollary}
...
Combined with LayerNorm, $\alpha \approx 2 - d\log\chi_k/d\log\sigma_k$, explaining MLP up-projections with $\alpha \approx 0.8$.
...
\subsection{The conv gap: GN vs.\ Kronecker}\label{sec:conv-gap}

For conv layers, the curvature ratio $\chi_k = h_k^{\mathrm{exact}}/h_k^{\mathrm{Kron}} \sim 10^3$--$10^7$ yet $\alpha \approx 2$. Although $\chi_k$ can be very large in magnitude, what matters for $\alpha$ is the \emph{log-slope} $d\log\chi_k/d\log\sigma_k \approx 0$, not the magnitude: the GN-to-exact gap from weight-sharing is approximately rank-independent. The \emph{concentration ratio} $\bar{h}^{\mathrm{SoP}}/\bar{h}^{\mathrm{PoS}} \approx 1$ confirms that Kronecker factorization is accurate; $\chi_k \gg 1$ reflects the GN approximation error (Appendix~\ref{app:conv-gap}).
...
\begin{proposition}[Spectral transfer relation]\label{thm:triangle}
Suppose $\sigma_k \sim k^{-\gamma}$ and $h_k = c\sigma_k^\alpha$ hold as effective power laws on a finite rank window $[k_1, k_2]$. Then directional curvature decays as $h_k \sim k^{-s}$ with $s = \alpha\gamma$ on that window.
\end{proposition}

\begin{proof}
$h_k = c(c' k^{-\gamma})^\alpha = c'' k^{-\alpha\gamma}$.
\end{proof}

\begin{remark}[Empirical content of the spectral transfer relation]\label{rem:not-tautology}
The relation $s = \alpha\gamma$ is algebraic under the local power-law ansatz---the one-line proof is intentional. Its empirical content is threefold: \textbf{(i)}~$\alpha$ and $\gamma$ are fit on \emph{different} data ($(\sigma_k, h_k)$ from HVPs vs.\ $(\log k, \log\sigma_k)$ from SVD), yet their product recovers $s$ from rank-ordered curvatures to median error \textbf{1.9\%} on CIFAR, \textbf{1.0\%} on Tiny-ImageNet, \textbf{1.6\%} on ImageNet-1K (Figure~\ref{fig:triangle}); \textbf{(ii)}~$\sigma_k$ is \emph{not} globally $k^{-\gamma}$---Bayesian Information Criterion (BIC) selects log-normal over power-law on most layers (Figure~\ref{fig:rank-profile}; Appendix~\ref{app:gamma-profile})---yet the effective exponent $\gamma$ on the top-$k$ window still produces a tight relation; and \textbf{(iii)}~the relation holds across 93 layers spanning five architectures and three datasets with no free parameters. A pure tautology produces no testable predictions; this relation is violated whenever the local power-law ansatz breaks down, and it \emph{does} break at boundary layers (Appendix~\ref{app:triangle}).
\end{remark}

\begin{figure}[t]
\centering
\includegraphics[width=0.58\linewidth]{fig2_alpha_triangle.pdf}
\caption{Measured directional curvature decay $s$ vs.\ predicted $s=\alpha\gamma$ across 93 layers and five architectures/datasets. Points near the diagonal validate the spectral transfer relation; shaded band is $\pm10\%$. Color indicates data source.}
\label{fig:triangle}
\end{figure}
...
\begin{corollary}[Connection to Heavy-Tailed Self-Regularization (HT-SR)]\label{cor:htsr}
Martin--Mahoney's weight tail exponent $\alpha_{\mathrm{weight}}$~\cite{martin2021implicit} is constrained by $s \approx \alpha\gamma$ governing directional curvature decay. We find $\alpha_{\mathrm{weight}} < s$ consistently, consistent with heavier weight tails correlating with better generalization.
\end{corollary}

\subsection{Near-rank-one curvature}

\begin{proposition}[Zeta function bound on curvature participation]\label{thm:rank-one}
Assuming $h_k \sim k^{-\alpha\gamma}$ extends over the measured rank window with $\alpha\gamma > 1$, the participation ratio $\mathrm{PR}_h = (\sum h_k)^2 / \sum h_k^2 \leq \zeta(\alpha\gamma)^2 / \zeta(2\alpha\gamma)$. For $\alpha\gamma = 2$: $\mathrm{PR}_h \lesssim 2.5$.
\end{proposition}

\noindent On the measured top-$k$ window, curvature is highly concentrated: $\mathrm{PR}_h \in [1.0, 1.8]$, suggesting that second-order information along a few gradient singular directions captures most of the layer-wise curvature mass. Gradients span moderate-dimensional subspaces ($\mathrm{PR}_{\mathrm{grad}} \in [1.8, 58]$; Appendix~\ref{app:pr-table}). Width scaling from $n=64$ to $512$ changes PR by only 0.7 (Appendix~\ref{app:pr-scaling}). The bound applies under the power-law ansatz; real spectra have finite-rank effects and non-power-law tails that limit its applicability (see BIC results, Appendix~\ref{app:gamma-profile}).

\subsection{From decomposition to Spectral Newton}\label{sec:optimizer-validation}

The decomposition predicts \emph{which layers} admit Kronecker-style curvature approximations: when $|\alpha - 2|$ is small and the concentration ratio $\bar{h}^{\mathrm{SoP}}/\bar{h}^{\mathrm{PoS}} \approx 1$, the Kronecker product-of-means matches the exact mean-of-products Hessian along gradient directions (Figure~\ref{fig:kfac-pred}). Conversely, layers with $\alpha \ll 2$ (LayerNorm + large output dimension) or $\alpha \gg 2$ (softmax heads) break alignment; full Hessian or task-specific preconditioning is required, consistent with K-FAC and Shampoo using richer structure than diagonal Adam.
...
\textbf{Directional curvature, not eigenvalues.} Throughout this work, $h_k$ denotes directional curvature (Rayleigh quotient) along gradient singular directions, not true Hessian eigenvalues. The spectral transfer relation $s \approx \alpha\gamma$ describes decay of these directional curvatures; connecting $s$ to the true Hessian eigenvalue spectrum requires showing that gradient singular directions approximate Hessian eigenvectors, which we verify empirically ($R^2 > 0.99$ for most conv layers, Appendix~\ref{app:hvp-table}) but do not prove in general.
\textbf{Scale.} Our largest models are GPT-2 (6-layer, 85M) and ResNet-50; the theory is architecture-generic and makes concrete, falsifiable predictions at larger scales---for instance, the decomposition predicts that ViT-Large attention layers will exhibit $\alpha \approx 1$ (driven by LayerNorm-induced alignment collapse, Section~\ref{sec:mechanisms}) and that LLaMA-7B MLP layers will follow $\alpha < 1.2$ with the same Kronecker decomposition---but we have not yet verified these. Validation at the 1--10B parameter regime is the most important next step.
\textbf{Spectral Newton (SN).} SN results (Table~\ref{tab:sn-vision}) are single-seed, single-architecture, and serve as directional evidence rather than a comprehensive optimizer comparison. Multi-seed evaluation with wall-clock and FLOP accounting, and comparison against K-FAC, Shampoo, and Muon, is needed.
\begin{enumerate}[leftmargin=*,nosep]
...
All empirical claims are reproduced from scripts that measure exact Hessian-vector products per layer. The full reproducibility package is available at \url{https://github.com/9D-Labs/9d-spectral-alignment-decomposition}:
\begin{itemize}[leftmargin=*,nosep]
    ...
    \item \textbf{Full reproduction:} \texttt{EXPERIMENTS.md} documents the measurement protocol for all 10 experiments (Modal A10G, $\sim$3--4\,h total).
\end{itemize}
...
\begin{tabular}{lccc}
\toprule
\textbf{Layer} & $\chi_k$ (curvature ratio) & Conc.\ ratio & $\alpha$ \\
\midrule
conv1 & $\sim 10^7$ & 0.88 & 1.99 \\
layer2.0.conv1 & $\sim 10^5$ & 0.98 & 2.57 \\
layer4.0.conv2 & $\sim 10^3$ & 0.59 & 2.23 \\
fc & $\sim 10^4$ & 0.88 & 1.64 \\
\bottomrule
\end{tabular}
\end{center}

\noindent Although the curvature ratio $\chi_k$ can be very large, its log-slope with respect to $\sigma_k$ is near zero for most layers, so it does not substantially shift $\alpha$ from the baseline $\beta \approx 2$. Note that layer2.0.conv1 has $\alpha = 2.57$, reflecting non-negligible $d\log\chi_k/d\log\sigma_k$ in that layer.
...
Interior conv layers move toward a stable high-$\alpha$ regime near~2 (layer2.conv2 reaches 2.12), while the boundary layer conv1 plateaus near 1.8. FC layers undergo a sharp early drop (1.99 $\to$ 1.05 by epoch~1) and remain below the conv regime throughout training, consistent with developing softmax concentration.
...
We compare three rank-ordered models for $\sigma_k$: power law $\sigma_k \propto k^{-\gamma}$, exponential $\sigma_k \propto e^{-\beta k}$, and log-normal in rank ($\log\sigma_k$ quadratic in $\log k$), using the Bayesian Information Criterion (BIC).
...
\subsection{LayerNorm (Proposition~\ref{thm:layernorm})}
